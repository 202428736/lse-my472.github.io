...
df <- read_csv("data/fb-congress-data.csv") # this is a relative file path (this is already saved as a tibble)
...
df <- df %>%
  filter(!post_type %in% c("music", "note")) %>% #filter out the value of post_type being one of these two values. 
...
```{r}
likes_hist + 
  scale_x_log10() #you can add 1 also to avoid losing the ones with 0 as a value of likes
```
...
2. the log transformation caused some values to be dropped  (yes the log 0)
