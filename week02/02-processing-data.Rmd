...
macroeconomic data are continuously revised. Thus, also for 2019 and 2020 the values might be slightly different now.
...
colnames(ip_df) <- str_replace(colnames(ip_df), "X", "") #we are removing the X in front of the column's names
head(ip_df)

```
This data frame is untidy, because each row is a country but each column is giving us the industrial production in a particular date. Each column is industrial production, so this is not tidy. It is currently wide and we want it long. We want 3 columns: country, date, industrial production.
...
```
This is also untidy because we have for the same country 6 rows (or more), plus for the same ip or unemployment also several rows... we have it in long format but we want it in wide format. 
...
#also can be written as: ip_long <- ip %>% pivot_longer(cols = colnames(ip)[2:length(colnames(ip))], names_to = "date", values_to = "ip")
ip_long
```
