...
line_plot <- ggplot(unemp) + geom_line(aes(x = date, y = unemp)) 
line_plot <- line_plot + scale_y_continuous(labels = scales::percent)

# Adding the rectangle geometry (set axes limits so that background covers entire plot)
my_plot <- line_plot +
  geom_rect(data=pres, 
            aes(xmin=start, xmax=end, fill = party),
            ymin = yrange[1]-0.05, ymax = yrange[2]+0.05) +
...
my_plot <- my_plot +
  geom_text(data=pres,
            aes(x=start, y=yrange[2]-0.01, label=name),
